...
ML Model Comparison
=========================================
This script compares the models Ridge, SVR, Keras NN 
and a linear model. All models contained optimized parameters
tested in another script. 

Warning: This script reads data from "PreparedDataAll.csv" and
returns results to "MLResults.csv". It will overwrite the output 
...
# Neural network (Keras) with pretrained hyperparameters
def run_NN(X,Y):
    dimension=X.shape[1]
    print(dimension)
...
# Support Vector Machine with pre-trained hyperparameters
def run_SVR(X,Y):
    dimension=X.shape[1]
...
# Output for results. Add flags to avoid overwriting output file. 
